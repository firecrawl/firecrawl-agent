...
import com.firecrawl.agent.model._
import org.json4s._
...
object JsonSupport extends SttpJson4sApi {
  def enumSerializers: Seq[Serializer[_]] = Seq[Serializer[_]]() :+
    new EnumNameSerializer(ModelConfigEnums.Provider) :+
    new EnumNameSerializer(RunRequestEnums.Format) :+
    new EnumNameSerializer(RunResponseEnums.Format) :+
    new EnumNameSerializer(SubAgentConfigEnums.Tools) :+
    new EnumNameSerializer(WorkerProgressEnums.Status)
