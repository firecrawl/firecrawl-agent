...
ModelConfig <- R6::R6Class(
  ...
  public = list(
    ...
    initialize = function(`provider` = NULL, `model` = NULL, ...) {
      if (!is.null(`provider`)) {
        if (!(`provider` %in% c("anthropic", "openai", "google", "gateway", "firecrawl", "acp"))) {
          stop(paste("Error! \"", `provider`, "\" cannot be assigned to `provider`. Must be \"anthropic\", \"openai\", \"google\", \"gateway\", \"firecrawl\", \"acp\".", sep = ""))
        }
    ...
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`provider`)) {
        if (!is.null(this_object$`provider`) && !(this_object$`provider` %in% c("anthropic", "openai", "google", "gateway", "firecrawl", "acp"))) {
          stop(paste("Error! \"", this_object$`provider`, "\" cannot be assigned to `provider`. Must be \"anthropic\", \"openai\", \"google\", \"gateway\", \"firecrawl\", \"acp\".", sep = ""))
        }
