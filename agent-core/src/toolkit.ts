 ...
 */
export function buildFirecrawlToolkit(firecrawlApiKey: string): Toolkit {
  const { systemPrompt, ...tools } = FirecrawlTools({ apiKey: firecrawlApiKey });

  return {
    tools,
    systemPrompt: systemPrompt ?? undefined,
    createFiltered: (enabled) => {
      const opts: Record<string, unknown> = { apiKey: firecrawlApiKey };
      if (enabled) {
