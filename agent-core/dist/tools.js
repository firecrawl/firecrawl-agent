...
const formatOutput = tool({
  description: "Format the final output as CSV, JSON, or text. Call this when you have collected all data and are ready to present results.",
  inputSchema: z.object({
    ...
    columns: z.array(z.string()).optional().describe("Column names for CSV format")
  }),
  execute: async ({ format, data, columns }) => {
    switch (format) {
      ...
      case "csv": {
        ...
        return {
          format: "csv",
          content: Papa.default.unparse(rows, { columns })
        };
      }
      case "text":
        return {
...
function setSharedBash(b) {
  g.__firecrawlBash = b;
}
async function initBashWithFiles(files) {
...
async function readBashFile(filePath) {
  const bash = getSharedBash();
  if (!bash) return "";
