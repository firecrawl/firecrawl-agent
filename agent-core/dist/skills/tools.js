...
function createSkillTools(skills, customInstructions) {
  const skillMap = new Map(skills.map((s) => [s.name, s]));
  const domainIndex = buildDomainIndex(skills);
  const catalogDescription = skills.length ? `Load a skill's full instructions. Available: ${skills.map((s) => `${s.name} (${s.description.slice(0, 60)})`).join("; ")}` : "Load a skill's instructions by name. No skills currently available.";
  const domainSkills = skills.filter((s) => s.sitePlaybooks?.length);
  const domainHint = domainSkills.length ? ` Site-specific playbooks auto-load via lookup_site_playbook for: ${domainSkills.flatMap((s) => s.sitePlaybooks.map((p) => p.platform)).join(", ")}.` : "";
  return {
    load_skill: tool({
...
${custom}`;
    ...
    lookup_site_playbook: tool({
      description: "Look up a site-specific navigation playbook by URL or domain. Returns detailed instructions for navigating that site (URL patterns, API endpoints, pagination, gotchas). Call this when you're about to scrape a site to check if a playbook exists.",
      inputSchema: z.object({
      ...
      execute: async ({ url }) => {
        ...
        const match = domainIndex.get(domain) ?? domainIndex.get(stripped) ?? [...domainIndex.entries()].find(([d]) => domain.endsWith(d) || stripped.endsWith(d))?.[1];
        if (!match) {
    ...
    read_skill_resource: tool({
      description: "Read a file from a skill directory. Use after loading a skill to access its scripts or reference docs.",
      inputSchema: z.object({
