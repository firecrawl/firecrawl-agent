...
defmodule FirecrawlAgentAPI.Connection do
  @moduledoc """
  Handle Tesla connections for FirecrawlAgentAPI.

  Additional middleware can be set in the compile-time or runtime configuration:

      config :tesla, FirecrawlAgentAPI.Connection,
        base_url: "http://localhost:3000/api/v1",
        adapter: Tesla.Adapter.Hackney

  The default base URL can also be set as:

      config :firecrawl_agent,
        :base_url, "http://localhost:3000/api/v1"
  """
  ...
  @doc """
  Configure a FirecrawlAgentAPI client.

  ### Parameters

  - `options`: an optional keyword list of FirecrawlAgentAPI.Connection.options.
  ...
  def middleware(options \\ []) do
    base_url =
      Keyword.get(
        options,
        :base_url,
        Application.get_env(:firecrawl_agent, :base_url, @default_base_url)
      )
    ...
    user_agent =
      Keyword.get(
        ...
        Keyword.get(
          tesla_options,
          :user_agent,
          "openapi-generator - FirecrawlAgentAPI 1.0.0 - elixir"
        )
      )
